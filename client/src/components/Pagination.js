...
const renderData = (passengersData)=>{
    return(
        <ul>
            {passengersData.map((d)=> 
            // <li key={d.id}> {d.volume} </li>)
            <li key={d.id}> The passenger having id {d.slice(d['_id'].length-5)} using {d.airline[0].name} airlines</li>)
            }
        </ul>
    )
}
